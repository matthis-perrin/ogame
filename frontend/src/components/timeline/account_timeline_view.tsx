...
export const AccountTimelineView: FC<{accountTimeline: AccountTimeline}> = ({accountTimeline}) => {
  ...
  const {start, currentAccount, buildItemTimelines} = accountTimeline;
  const transitions = buildItemTimelines.reduce(
    (all, current) => all.concat(current.transitions),
    [] as TransitionnedAccount[]
  );
  const totalTime = currentAccount.currentTime;
  return (
    <Wrapper>
      <Header>
