...
export const Empire: FC<EmpireProps> = ({account}) => {
  ...
  for (const fleetId in account.fleets) {
    if (account.fleets.hasOwnProperty(fleetId)) {
      ...
      const inFlight = inFlightResources.get(fleet.destinationCoords) ?? {
      ...
      inFlightResources.set(fleet.destinationCoords, inFlight);
    }
  }
            ...
            if (account.planetDetails.hasOwnProperty(p.id)) {
              ...
              if (planet.resources.energy < 0) {
                ...
                const satNumber = planet.technologies.hasOwnProperty(SolarSatellite.id)
                  ...
                  : 0;
                requiredSat =
                  (satNumber as number) + Math.ceil(-planet.resources.energy / satEnergy);
              }
              return (
                <tr key={p.id}>
                  <td>
                  ...
                  <td>
                    {(() => {
                      const inFlight = inFlightResources.get(p.coords) ?? {
                        metal: 0 as ResourceAmount,
                        crystal: 0 as ResourceAmount,
                        deuterium: 0 as ResourceAmount,
                        sum: 0 as ResourceAmount,
